...
.``__init__``
****
O metedo ``__init__`` que é o construtor da classe. Ela é responsável por definir as dimensões da janela e da lógica do jogo, cria uma superfície para renderização e carrega os recursos necessários.
[source,ruby]
----
def __init__(self):
        pygame.init()
